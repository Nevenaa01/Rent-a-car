...
*/
	...
	public ArrayList<RentacarObjekat> dobaviSve(){
		ArrayList<RentacarObjekat> otvoreni = new ArrayList<RentacarObjekat>();
		ArrayList<RentacarObjekat> zatvoreni = new ArrayList<RentacarObjekat>();
		ArrayList<RentacarObjekat> trazeniObjekti = new ArrayList<RentacarObjekat>();
		
		for(RentacarObjekat objekat : objekti.values()) {
			Double pocetakRadnogVremena = Double.parseDouble(objekat.getRadnoVreme().split(" - ")[0]);
			Double krajRadnogVremena = Double.parseDouble(objekat.getRadnoVreme().split(" - ")[1]);
			...
			System.out.println(pocetakRadnogVremena + " " + trenutnoVreme + " " + krajRadnogVremena);
			
			if(pocetakRadnogVremena <= trenutnoVreme && trenutnoVreme <= krajRadnogVremena) {
				otvoreni.add(objekat);
			}
			else {
				zatvoreni.add(objekat);
			}
		}
		
		for(RentacarObjekat objekat : otvoreni) {
			trazeniObjekti.add(objekat);
			System.out.println(objekat.getId());
		}
		
		for(RentacarObjekat objekat : zatvoreni) {
			trazeniObjekti.add(objekat);
			System.out.println(objekat.getId());
		}
	...
	public ArrayList<RentacarObjekat> sort(String parametar, boolean rastuciRedosled, String vrstaVozila, String tipGoriva, boolean otvoreno, String nazivObjekta, String tip, String lokacija, Double prosecnaOcena){
		ArrayList<RentacarObjekat> trenutnaLista;
		if(!(vrstaVozila.equals("defaultna") && tipGoriva.equals("defaultna") && otvoreno == false)) {
			trenutnaLista = filtriraj(vrstaVozila, tipGoriva, otvoreno);
			System.out.println("bio sam ovde");
		}
		else {
			trenutnaLista = new ArrayList(dobaviPretrazene(nazivObjekta, tip, lokacija, prosecnaOcena));
	...
	public void azuriraj(RentacarObjekat o) {
		for(RentacarObjekat objekat : objekti.values()) {
			objekat.setOcena(o.getOcena());
