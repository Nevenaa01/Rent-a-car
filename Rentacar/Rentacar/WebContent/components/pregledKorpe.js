Vue.component("pregledKorpe", {
	data: function () {
		return {		
			...
			objekti: {},
			ukupnaCena: 0,
			porudzbina: {id: null, vozila: [], objekti: [], datumIznajmljivanja: null, trajanjeNajma: null, cena: null, kupac: null, korisnickoImeKupca: null, status: null},
			prikazanoDugme: true,
			kupac: {},
			popust: 0
		}
	...
	template: ` 
 	<div class="pozadina">
		...
		<table class="styled-table"  v-if="this.ulogovan">
			<thead>
				<tr>
			...
			<tbody>
				...
				<tr class="poslednjiRed">
					...
					<td>Ukupna cena</td>
					<td>= {{ukupnaCena}}</td>
					<td colspan="2"><button type="submit" v-if="vozila.length > 0" v-on:click="napraviPorudzbinu" class="btn btnSearch">Poruči</button></td>
				</tr>
			</tbody>
		</table><br><br>
	...
	methods: {
		...
		napraviPorudzbinu : function(){	
			var j = 0;		
			for(let i = 0; i < this.vozila.length; i++){
				var br = this.kolicine[i]
				while(br > 0){
					this.porudzbina.vozila[j++] = this.vozila[i];
					br--;
				}
				
			}
			this.porudzbina.datumIznajmljivanja = this.$route.params.pocetniDatum;
			...
			this.porudzbina.kupac = this.korisnik.ime + " " + this.korisnik.prezime;
			this.porudzbina.korisnickoImeKupca = this.korisnik.korisnickoIme;
			...
			axios
				...
				.then(response => {
					...
					var bodovi = this.ukupnaCena/1000 * 133
					return axios
						.put('rest/kupac/dodajBodove/' + this.korisnik.korisnickoIme + '/' + bodovi)
				})
				.then(
					alert("Uspesno dodata porudžbina")
				)
	...
	mounted() {
		axios.post('rest/korisnik/ulogovan')
			...
			.then(response =>
			{
				...
				return axios.get('rest/kupac/dobaviPoId/' + this.korisnik.id)
			})
			.then(response =>{
				this.kupac = response.data
				
				if(this.kupac.tipKupca === "srebrni"){
					this.popust = 4
					this.ukupnaCena = this.ukupnaCena * (1 - this.popust/100)
				}
				else if(this.kupac.tipKupca === "zlatni"){
					this.popust = 10
					this.ukupnaCena = this.ukupnaCena * (1 - this.popust/100)
				}
			})
