Vue.component("prikazPorudzbinaKupca", {
				...
				<table class="styled-table">
						<thead>
							<tr>
								...
								<th class="centralnoPoravnanje">Prikaz 	</th>
						...
						<tbody>
							<template v-for="(porudzbina, index) in porudzbine">
						      ...
						      <td class="logoTd">
								...
								<td ><button type="submit" v-on:click="PrikaziPorudzbinu(porudzbina.id)" class="btn btnSearch">Prikazi celu porudzbinu</button></td>
								<td><button type="submit" v-on:click="ostaviKomentar(index)" class="btn btnSearch btnKolicina">+ <i class="fa fa-comments"></i></button></td>
						      </tr>
						    </template>
						</tbody>
					</table><br><br>
	...
	methods: {
		...
		PrikaziPorudzbinu:function(idx)
		{
			router.push(`/prikazPorudzbineKupac/${idx}`)
		},
		ostaviKomentar : function(i){
			if(this.porudzbine[i].status === "vraceno"){
				this.prikazFormeZaKomentare = true;
				this.objekat = this.porudzbine[i].objekat
			}
			else{
				alert("Mozete ostaviti komentare samo za vozila koja su vracena");
			}
		},
