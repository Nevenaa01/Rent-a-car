Vue.component("DodavanjeVozila", {
	...
	<br><br>
	         ...
	         <div class="col-sm-12 d-flex justify-content-center align-items-cente" v-if="this.ulogovan">
	                <button class="btn btnSearch" value="Kreiraj vozilo" v-on:click="kreiraj()">Kreiraj vozilo</button>
	        </div>
<br><br> 
		</div>
	...
	methods: {
		kreiraj:function()
		{
			...
			this.v.idObjekta=this.id;
			axios.post('rest/vozilo/kreiraj',this.v)
			.then(response=>{
				const valid=response.data
				if(valid)
				{
					alert("Uspesno izmenjeno u obj");
				}
					
				else
					alert("Neuspesno izmenjeno u obj");
				return axios.get('rest/rentACarObjekat/kreirajvozilo/'+this.id)
			}).then(response=>{
				const valid=response.data
				if(valid)
				{
					...
					alert("Neuspesno kreirano");
			})
		}
	},
	mounted() {
		this.id = this.$route.params.id
			axios.post('rest/korisnik/ulogovan').then(response => 
			{
