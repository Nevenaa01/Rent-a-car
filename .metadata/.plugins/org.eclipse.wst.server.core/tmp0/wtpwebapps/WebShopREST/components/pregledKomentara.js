Vue.component("pregledKomentara", {
 	...
 	<div class="pozadina">
		...
		<br><br><br>
			...
			<table class="styled-table">
					<thead>
						<tr>
							<th class="centralnoPoravnanje">Korisnik</th>
							<th class="centralnoPoravnanje">Ocena</th>
							<th class="centralnoPoravnanje">Komentar</i></th>
							<th class="centralnoPoravnanje">Odobri</th>
							<th class="centralnoPoravnanje">Odbij</th>
						</tr>
					</thead>
					<tbody>
						<tr v-for="(k, index) in komentari">
							<td>{{k.imePrezimeKorisnika}}</td>
							<td class="centralnoPoravnanje">{{k.ocena}}</td>
							<td>{{k.komentar}}</td>
							<td><button type="submit" v-on:click="azurirajStatus(k.id, index, 'odobreno')" class="btn btnSearch">Odobri</button></td>
							<td><button type="submit" v-on:click="azurirajStatus(k.id, index, 'odbijeno')" class="btn btnSearch">Odbij</button></td>
						</tr>
					</tbody>
				</table><br><br>  
				<div class="container-fluid">
	...
	,
	methods: {
		azurirajStatus : function(id, indx, status){
			axios
				.put('rest/komentar/azuriraj/' + id + '/' + status)
				.then(response =>{
					alert("Zabelezena akcija")
					this.komentari.splice(indx,1)
				})
		}
	},
	mounted() {
		axios
			...
			return axios.get('rest/menadzer/dobaviMenadzera/' + this.korisnik.id)	
		})
		.then(response => {
			this.menadzer = response.data;
			
			return axios.get('rest/komentar/dobaviKomentareNaCekanjuZaObjekat/' + this.menadzer.rentacarObjekat)
		})
		.then(response =>{
